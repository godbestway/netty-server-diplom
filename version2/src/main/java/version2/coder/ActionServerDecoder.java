 ...
 * @Date: 25.01.2021 15:34
 * @Description:
 */
public class ActionServerDecoder extends ByteToMessageDecoder {
    protected static Logger logger = LoggerFactory.getLogger(ActionServerDecoder.class);

    protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf in, List<Object> out) throws Exception {
        logger.info("receive a  action object"+System.currentTimeMillis());
    ...
    public MessageLite decodeBody(byte dataType, byte[] array, int offset, int length) throws Exception {
        if (dataType == 0x00) {
            ...
            return ActionGetPerflowAckMsgProto.ActionGetPerflowAckMsg.getDefaultInstance().getParserForType()
                    .parseFrom(array, offset, length);
        }else if(dataType == 0x0A){
            return ActionPutPerflowAckMsgProto.ActionPutPerflowAckMsg.getDefaultInstance().getParserForType()
                    .parseFrom(array, offset, length);
        }
        else if(dataType == 0x12){
            return ActionStateProto.ActionState.getDefaultInstance().getParserForType()
                    .parseFrom(array,offset,length);
        }
